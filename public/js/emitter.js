...
ee.on("newtrack", function (track) {
  for (var i = 0; i < uploadedFiles.length; i++) {
    ee.emit("newtrack", uploadedFiles[i]);
  }
});

ee.on("newtrack", function (track) {
  playlist.load([track]).then(function () {
    // Optional: You can perform additional actions after the track is successfully loaded
    console.log("Track loaded:", track);
  }).catch(function (error) {
    // Optional: Handle any errors that occur during track loading
...
var fileInput = document.getElementById("audioFilesInput");
...
function displayDownloadLink(link) {
  ...
  var $link = $("<a/>", {
    ...
    'text': 'Download SL ' + dateString,
    'class': 'm-2 btn btn-small btn-download-link'
  });
